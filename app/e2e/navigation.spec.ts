...
test.describe('トップページ', () => {
  test('タイトルと撮影ボタンが表示される', async ({ page }) => {
    ...
    await expect(page.getByRole('link', { name: '役一覧' })).toBeVisible();
  });
  ...
  test('「役一覧」リンクで /rules に遷移する', async ({ page }) => {
    await page.goto('/');

    await page.getByRole('link', { name: '役一覧' }).click();

    await expect(page).toHaveURL('/rules');
  });
});

test.describe('役一覧ページ', () => {
  test('ルールが一覧表示される', async ({ page }) => {
    await page.goto('/rules');

    await expect(page.getByRole('heading', { name: '役一覧' })).toBeVisible();
  ...
  test('「ホーム」ボタンでトップに戻る', async ({ page }) => {
    await page.goto('/rules');
